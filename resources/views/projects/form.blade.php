<x-layouts.app title="{{ isset($project) ? 'Edit Project' : 'Create Project' }}">
    <div class="max-w-2xl mx-auto py-2">
        ...
        <div class="bg-white p-8 rounded-3xl shadow-sm border border-neutral-200/60">
            ...
            <p class="text-sm text-neutral-500 mb-8">Fill in the fields below to publish or update your development project work.</p>
            ...
            <form action="{{ isset($project) ? route('projects.update', $project) : route('projects.store') }}"
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" required value="{{ old('name', $project->name ?? '') }}" class="w-full border {{ $errors->has('name') ? 'border-red-400 focus:ring-red-500' : 'border-neutral-200 focus:ring-neutral-900' }} bg-neutral-50/50 rounded-xl p-3 text-sm focus:ring-1 focus:outline-none transition text-neutral-900 placeholder-neutral-400" placeholder="e.g. E-Commerce Platform API">
                    @error('name')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label for="project_type_id" class="block text-sm font-semibold text-neutral-800 mb-1.5">Project Framework / Stack</label>
                    <select name="project_type_id" id="project_type_id" required class="w-full border {{ $errors->has('project_type_id') ? 'border-red-400 focus:ring-red-500' : 'border-neutral-200 focus:ring-neutral-900' }} bg-neutral-50/50 rounded-xl p-3 text-sm focus:ring-1 focus:ring-neutral-900 focus:outline-none transition text-neutral-900">
                        <option value="" disabled {{ !isset($project) ? 'selected' : '' }}>Select Tech Stack Domain...</option>
                ...
                <div class="border-t border-neutral-200/60 pt-6" x-data="{ items: [ {id: Date.now()} ] }">
                    <div class="flex justify-between items-center mb-4">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500">Upload additional application screenshots individually.</p>
                        </div>
                    ...
                    <div class="space-y-3 mb-2">
                        <template x-for="(item, index) in items" :key="item.id">
                            <div class="p-4 bg-neutral-50 border border-neutral-200/60 rounded-2xl relative flex items-center justify-between">
                                <div class="flex-1">
                                    <label class="block text-xs font-semibold text-neutral-700 mb-1.5">Screenshot File</label>
                ...
                <div class="flex justify-end pt-4 border-t border-neutral-100">
                    <button type="submit" class="bg-neutral-900 text-white font-medium text-sm py-2.5 px-6 rounded-xl hover:bg-neutral-800 transition shadow-sm tracking-wide">
                        {{ isset($project) ? 'Update Development Project' : 'Publish Project Workspace' }}
                    </button>
                </div>
            </form>
        </div>
