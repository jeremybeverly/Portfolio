...
<div class="group bg-white flex flex-col w-full border border-neutral-200 rounded-3xl overflow-hidden shadow-sm transition-all duration-300 hover:shadow-xl hover:-translate-y-1">

    @if($work->cover_image)
        <a href="{{ route('portfolio.designs.show', $work) }}" class="block w-full aspect-[4/3] overflow-hidden bg-neutral-100 border-b border-neutral-100 shrink-0">
    ...
    <div class="p-6 flex flex-col items-start text-left flex-1">
        ...
        @if($work->designWorkType)
            <span class="inline-flex items-center bg-neutral-100 border border-neutral-200 text-neutral-800 text-xs font-semibold px-3 py-1.5 rounded-full">
                ...
                {{ $work->designWorkType->name }}
            </span>
        @endif

        <a href="{{ route('portfolio.designs.show', $work) }}" class="block group/title mt-4">
        ...
        <p class="mt-2 mb-6 text-sm text-neutral-500 leading-relaxed line-clamp-3">
            {{ $work->description }}
        </p>

        <a href="{{ route('portfolio.designs.show', $work) }}" class="mt-auto inline-flex items-center text-white bg-neutral-900 hover:bg-neutral-800 transition-colors duration-200 font-medium leading-5 rounded-xl text-sm px-5 py-2.5 shadow-sm">
            Read more
