    ...
    <div class="max-w-2xl mx-auto py-2">
        ...
        <div class="bg-white p-8 rounded-3xl shadow-sm border border-neutral-200/60">
            ...
            <form action="{{ isset($designWork) ? route('designs.update', $designWork) : route('designs.store') }}"
                ...
                <div>
                    <label for="name" class="block text-sm font-semibold text-neutral-800 mb-1.5">Design Title</label>
                    <input type="text" name="name" id="name" required value="{{ old('name', $designWork->name ?? '') }}" class="w-full border {{ $errors->has('name') ? 'border-red-400 focus:ring-red-500' : 'border-neutral-200 focus:ring-neutral-900' }} bg-neutral-50/50 rounded-xl p-3 text-sm focus:ring-1 focus:outline-none transition text-neutral-900">
                    @error('name')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label for="design_work_type_id" class="block text-sm font-semibold text-neutral-800 mb-1.5">Type</label>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" required rows="4" class="w-full border {{ $errors->has('description') ? 'border-red-400 focus:ring-red-500' : 'border-neutral-200 focus:ring-neutral-900' }} bg-neutral-50/50 rounded-xl p-3 text-sm focus:ring-1 focus:ring-neutral-900 focus:outline-none transition text-neutral-900 " >{{ old('description', $designWork->description ?? '') }}</textarea>
                    @error('description')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>

                <!-- Primary Cover Thumbnail -->
                <div>
                    <label for="cover_image" class="block text-sm font-semibold text-neutral-800 mb-1.5">Thumbnail / Cover</label>
                        ...
                        <template x-for="(item, index) in items" :key="item.id">
                            <div class="p-4 bg-neutral-50 border border-neutral-200/60 rounded-2xl relative flex flex-col gap-4">
                                ...
                                <div class="w-full md:w-3/4 pr-12">
                                    <label class="block text-xs font-semibold text-neutral-700 mb-1.5">Pictures / Screenshot</label>
                                ...
                                <div class="w-full">
                                    <label class="block text-xs font-semibold text-neutral-700 mb-1.5">Image Description</label>
                                    <input type="text" :name="'gallery[' + index + '][caption]'" class="w-full border border-neutral-200 bg-white rounded-xl p-2.5 text-sm focus:ring-1 focus:ring-neutral-900 focus:outline-none transition text-neutral-900 placeholder-neutral-400">
                                </div>
                            </div>
                        </template>
                    </div>
                ...
                <div class="flex justify-end pt-4 border-t border-neutral-100">
                    <button type="submit" class="bg-neutral-900 text-white font-medium text-sm py-2.5 px-6 rounded-xl hover:bg-neutral-800 transition shadow-sm tracking-wide">
                        {{ isset($designWork) ? 'Update Creative Work' : 'Publish' }}
                    </button>
                </div>
            </form>
        </div>
